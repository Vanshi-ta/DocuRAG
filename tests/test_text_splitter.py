"""
Automated tests for src/ingestion/text_splitter.py.

These tests build synthetic LangChain Document objects directly (no PDF
required), so they run fast and don't depend on any file being present in
data/uploads/.
...
def make_document(text: str, page_number: int = 1, source_filename: str = "test.pdf") -> Document:
    return Document(
        page_content=text,
        metadata={
...
def test_validate_chunk_params_rejects_overlap_equal_to_size():
    with pytest.raises(ValueError, match="must be smaller"):
        validate_chunk_params(chunk_size=500, chunk_overlap=500)
...
def test_validate_chunk_params_accepts_reasonable_values():
...
def test_split_documents_produces_more_chunks_than_input_for_long_text():
    long_text = "This is a sentence about RAG systems. " * 200  # ~7,800 chars
...
def test_chunks_get_new_chunk_specific_metadata():
...
def test_no_chunk_exceeds_requested_chunk_size_by_much():
    # RecursiveCharacterTextSplitter can occasionally exceed chunk_size
    # slightly when no good separator is found, but chunks should never be
    # wildly larger than requested.
    doc = make_document("word " * 2000)  # long text, mostly spaces

    result = split_documents([doc], chunk_size=300, chunk_overlap=30)

    for chunk in result.chunks:
        assert len(chunk.page_content) <= 300 + 50  # small tolerance
